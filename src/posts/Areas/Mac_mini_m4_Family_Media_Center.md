...
文字使用 列表组织 是方便直接按级别跳跃查看，不可避免的造成内容过长，请使用右侧目录功能跳转
...
- [Jellyfin](https://jellyfin.org/)
  ...
  - DLNA 插件
    - 提供给不适合安装客户端的地方使用，不如 VR 眼镜 Quest3 上的播放器
...
- [IYUU](https://doc.iyuu.cn/)
  ...
  - 辅种工具
  - 辅种的操作就是：很多一样的资源会被发布到不同的 PT 站点，只需要下载一次就可以用于多个种子作种
  - 还能用于转移种子，比如用 qBittorrent 下载之后转移到 Transmission 下载（因为 Tr 比起 qB 更适合做很多种，占用少，可以根据种子数量和机器性能决定用不用转移）
...
- PT
  ...
  - 想慢慢玩
    - 慢慢收集小站 逐步走向大站
      - 小站会不定期开放注册，可以从 PT 贴吧/ 站点 Telegram ✈️ 群获开放注册取消息
    ...
    - 对于有考核的站点，先利用已有资源辅种，再下载免费资源（上传少下载多的更有利于做种的时候有上传量），保种赚魔力换上传
    - 刷上传量、辅种 MP 里面都有对应插件可以定时任务自动，不耗费什么精力
      - 前提是懂得这些操作的意义，并配置好，配置失误的结果一半是走向速通
  - 花钱把 PT 当 BT 用
    - 可以研究一下 捐赠入大站 + 盒子刷上传
...
- 电视选的 LG
  ...
  - 买工程遥控器（MKJ39170828）可以转外区装 APP
    - Jellyfin 客户端（其实支持 DLNA 但是没海报墙）
    - Netflix 客户端
      - 等等主流流媒体客户端
  - 自带浏览器可以局域网直接访问 MP 和 qB 等软件的 WebUI 控制订阅和下载等操作
    - LG 的遥控器可以靠指向控制光标 比只能按上下左右的那种遥控器控制网页方便很多 ![LG 遥控](/assets/blog_images/mac_mini/LG_TV.gif)
